...
| `docs/research_improvements/` | An audit of all three packages against the Fabro source: what we hand-roll that Fabro already does, four operator-observed gaps traced to Fabro lines, and nine settled dead ends. A plan, not a changelog: one item has been applied — the stall watchdog that made `watch_checks`'s own timeout unreachable (`01`, item 2, applied 2026-09-22 after it cost seven runs). Everything else is still a proposal. `00-overview.md` first. |
