...
| `docs/scheduler/` | The coder scheduler: an external service that owns admission to the two llama.cpp instances, because fabro's own queue is FIFO-on-creation with no priority and no per-pool concurrency. `00-overview-and-contracts.md` first — it carries 19 settled operator decisions and the 11 source findings behind them, then 14 numbered task drafts. Each draft is written to be implementable from the overview plus its own file plus this repository. **Drafts 01–13 are built and deployed, and draft 14's 24-hour shakedown window is running from 2026-09-19T15:29:13Z** — its acceptance criteria are pending, not unstarted. The `claim` regression that killed the first bring-up is fixed (`5fa974d`, the `mkdir -p /tmp/fabro` that died with `acquire`) and two runs have cleared that node since. `ops/test-task-gates.sh` now covers `claim` and `mark_stuck`, so the same class of shell-level regression fails offline instead of costing two coder boxes four hours each. Draft 13 turned the four `backlog-<repo>` schedules off, so the scheduler is the only producer of `backlog` runs; `ops/fabro-automation-schedule.sh` is the way back on, and `ops/fabro-fire-backlog.sh` is the manual escape hatch. The service itself is `ops/scheduler/`; its LAN page is `http://10.10.0.32:32280/`, and `docs/scheduler/OPERATING.md` is the operator's quickstart — the page, the three controls, changing repo priority (a rebuild), and the two monitor conditions. |
...
```sh
./ops/test-task-gates.sh      # 137 checks, offline — no host, container or network
```
...
shared review-merge graph's `merge`.
...
| The `merge` node waits for `mergeable` to leave `UNKNOWN` and retries a branch-moved rejection | Fabro checkpoints (commit + push) after every stage, and `merge` is the one node whose branch is what is being merged — the push lands ~15ms before it starts and invalidates GitHub's computed merge ref, which is rejected as `Base branch was modified` even though the base has not moved. A pure race: `jelly-swipe#386` lost it with `main` unmoved for 35h and no protection rules, while run `01M2TFEHPYXXTQST6VPXREA854` won it with the same 10ms gap. Without the wait and the retry a mergeable PR is reported to a human as a permissions or conflict problem. |
