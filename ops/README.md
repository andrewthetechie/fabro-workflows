...
| `docker-compose.yaml` | Runs both containers: the `fabro` server and the `scheduler` (the coder scheduler, built from `./scheduler/`). It is also the only place the scheduler's port, volume, and — since the task/stage columns — its read-only `/var/run/docker.sock` mount are declared. |
