...
| File | Purpose |
|---|---|
| `fabro-branch-sweep.sh` | Deletes leaked `fabro/run/*` and `fabro/meta/*` branches from the target repos (daily cron). 0.362 no longer creates `fabro/meta/*`; it still sweeps the ones earlier versions left. Deterministic; see its header comments. |
...
| `docker-compose.yaml` | Runs both containers: the `fabro` server and the `scheduler` (the coder scheduler, built from `./scheduler/`). It is also the only place the scheduler's port and volume are declared; since the fabro-upgrade H6 change it no longer mounts the host docker socket. |
...
| `scheduler/` | The **coder scheduler**: a FastAPI service that owns admission to the two coder instances, so four uncoordinated automations stop fighting over two single-slot boxes. Reads `repos.toml`, inventories `agent`-labelled issues from GitHub with ETag caching, ranks them, serves the queue on `/`, and leases a coder instance to the highest-ranked issue whose repo has no run in flight and starts that run by itself. The coder-instances table also shows each leased run's decomposed-task count and current stage — a 30s background probe reads the sandbox's `/tmp/fabro/tasks.json` and the stage from `/runs/{id}/stages`, both through the fabro API (no docker socket; that read moved off the socket in the fabro-upgrade H6 change). `POST /api/dispatch-once` remains as the manual override. Deploy and verify under *The coder scheduler* below. |
...
- Discord webhook URL: `/storage/secrets/discord_webhook_url` in the container volume.
  `discord-notify.sh`'s `fallback` kind reuses it for the model-escalation alert.
...
pause and cancel controls; the queue itself is reorderable — drag a row, or its
up/down/to-top buttons — so the operator sets the exact order, and the running-instances
table shows a time-left **ETA** (subtask rate, else the repo's own average). Draft 15 adds
the per-run local-vs-hosted token and cost split on the history page. `POST
/api/dispatch-once` stays as the operator's manual
...
`2026-09-19T15:29:13Z`. It was first armed at `14:55:27Z`; deploying the follow-ups below
...
| `scheduler/src/fabro_scheduler/store.py` | SQLite at `/data/scheduler.db`: `issue_cache` (one row per queue item), `repo_etag` (per-repo ETag, attempt/success times, last error), `leases` (one row per box currently holding a run, `coder_pool` as the primary key), `repo_affinity` (the box each repo last ran on), draft 11's `overrides` (now the explicit queue order: a row per item the operator placed, `set_queue_order` writes ranks 0..N-1 in one transaction) and `pool_state` (the **Drain** flag per box). `run_history` carries the local/hosted usage split columns, added to an existing DB by `_apply_migrations` (several `ALTER TABLE ... ADD COLUMN`). `first_seen` is written once and **never** rewritten — an issue could otherwise be refreshed forever and never reach the starvation ceiling. **Only a successful fetch writes a cache row**: a failed one keeps the cached items and the ETag, so an unreachable GitHub does not look like an empty queue. The lease, override and pool-state tables are the deliberate exception — they are not inventory. The `leases` primary key is what makes double-dispatch to one box impossible: the database enforces it, not the loop. |
...
| `scheduler/src/fabro_scheduler/templates/queue.html` | The page: the **coder instances** (state, lease, ETA, and the Drain/Undrain and Cancel-run controls), the queue with per-row up/down/to-top controls and drag-and-drop reordering, and a per-repo freshness strip. Inline CSS and a few lines of inline JS, no external assets, refreshes itself on the same 60s beat. Each control is a real `<form method="post">`, so the buttons work with scripting off (the response is raw JSON); with scripting on the submit is intercepted, the page reloads on success and shows the error otherwise. |
| `scheduler/src/fabro_scheduler/app.py` | `/`, `/api/queue`, `/api/repos`, `/api/pools`, `/health`, `POST /api/dispatch-once`, the reorder routes `POST /api/queue/{repo}/{issue}/top|up|down` and `POST /api/queue/reorder` (the old `/bump` is kept), and `POST /api/pools/{pool}/drain`, `/undrain` and `/cancel`. Supports the ETA column (`estimate_remaining`) and the history token columns. `main` starts the inventory poller, the dispatch loop **and** the 15s release poll; every one is off in any other construction, so a test drives a tick by hand. `dispatch-once` takes `repo`, `issue_number` and `coder_pool`, refuses a pool already leased with a `409`, records the lease on the way out, and answers `503` with no `FABRO_API_TOKEN`. Every mutating route is `POST` and every one logs what it changed, because there is no auth to attribute it to. |
| `scheduler/src/fabro_scheduler/reconcile.py` | Draft 09's release/requeue/recovery, plus draft 11's one change to it: a cancel requeues. `should_requeue` keys on the failure **reason** for the two failures whose category cannot decide — `terminated` (a fabro restart, category `deterministic`) and `cancelled` (the operator's cancel, category `canceled`, one L). Both are in the run projection, so neither costs the extra events call. The `transient_infra` category path had never fired once: `last_failure_category` filtered on `event["type"] == "run.failed"` and the deployed server sends `event`, `stage.failed` and `failure.category` — three mismatches, fixed 2026-09-19, live-verified returning `deterministic`/`transient_infra` on real runs. Repairing it armed a loop, so `MAX_REQUEUES = 3` now bounds it: a requeue restores the issue at the FRONT of the queue (original `first_seen`), and fabro classifies a flaky unit test as `transient_infra`, so a recurring failure would otherwise hold a coder box forever. Past the budget the issue is released without a requeue and waits for a human. Two cadences share one thread: the fabro pass every **15s** (one cheap `GET /runs/{id}` per held lease) and the GitHub receipt scan every **10 minutes** (four uncached requests, 24/hour against 5,000). The receipt scan used to run only at startup, on the premise that only a restart could orphan a receipt; 2026-09-19 disproved that, so it repeats — and because `_dispatch` writes the receipt seconds before the run exists, a receipt must look orphaned in **two consecutive scans** before anything is written. |
...
# Reorder the queue. Each one persists an explicit order (the operator's order is a
# ranked prefix; a new item lands at the back in auto order until moved). Ordering only:
# never pre-empts a running lease. Cleared when the item is dispatched.
curl -fsS -X POST "$B/api/queue/$R/123/top"      # move to the very front (was "Next")
curl -fsS -X POST "$B/api/queue/$R/123/up"       # one position earlier
curl -fsS -X POST "$B/api/queue/$R/123/down"     # one position later
# Drag-and-drop: POST the whole queue in the dragged order.
curl -fsS -X POST "$B/api/queue/reorder" \
  -H 'Content-Type: application/json' \
  -d '{"items": [{"repo": "andrewthetechie/jelly-swipe", "issue_number": 123}, ...]}'
# The original single-item bump still works (same as top for one item).
# curl -fsS -X POST "$B/api/queue/$R/123/bump"   # {"override_rank":-n}
