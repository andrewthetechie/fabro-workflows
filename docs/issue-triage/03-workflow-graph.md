...
Probe it before building on it. A throwaway two-node graph: a hexagon with one
`freeform=true` edge into a command node with `stdin_source="human.gate.text"` whose
script is `cat > /tmp/fabro/probe.txt; wc -c /tmp/fabro/probe.txt`. Run it, answer with a
known string, read the stage log.

- If the text arrives, use `human.gate.text` as written below.
- If it does not, try `human.gate.ask_human.answer`, then `human.gate.selected`.
- If none of them arrive, **delete the `freeform=true` edge** and relabel the option
  `[A] I answered on the issue — re-read it`, dropping `record_answer`'s `cat` and
  keeping only its re-fetch. The answer then travels through GitHub, which is where it
  has to end up anyway. Record the outcome in this file either way.

This is the same discipline that caught `${X}` not interpolating in
`[run.environment.env]`: probe the mechanism, do not reconstruct it from the docs.
...
jq -nc --arg r \"$R\" --arg q \"$Q\" --arg qt \"$QT\" --arg a \"$ANS\" '{context_updates:{triage_readiness:$r,question_count:$q,triage_questions:$qt,answered:$a}}'"]
    ...
    record_answer [label="Record the human answer", shape=parallelogram, output_schema="routing",
        ...
        script="cat > /tmp/fabro/human_answer.md
N=$(jq -r .number /tmp/fabro/issue.json)
